...
export function CleanInjectPage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const res = await api.cleanInject(f, payload as Record<string, string>);
  ...
  const processLabel = () => {
    if (busy) return 'Обрабатываю...';
    if (files.length <= 1) return 'Очистить и внедрить';
    if (activeFileIndex < files.length - 1) {
      return `Очистить и внедрить — следующий (${activeFileIndex + 1}/${files.length})`;
    }
    return `Очистить и внедрить — завершить (${files.length}/${files.length})`;
  };
  ...
  return (
    <div className="page">
      <div className="page-header">
        <h1>Clean + Inject</h1>
        <p className="muted">
          Полный пайплайн для офферов от маркетологов: сначала чистим, потом инжектим.
          Несколько ZIP обрабатываются по очереди; в конце — общий список результатов.
        </p>
      </div>
      ...
      <div className="card">
        ...
        {files.length > 1 && !allDone && (
          <p className="form-hint dim small" style={{ marginTop: '0.75rem' }}>
            В очереди {files.length} архивов. После каждого шага при необходимости поправь параметры перед следующим.
          </p>
        )}
        {allDone && (
          <p className="form-hint dim small" style={{ marginTop: '0.75rem' }}>
            Все архивы обработаны. Скачай результаты ниже или нажми «Сброс».
          </p>
        )}
      </div>

      <div className="card">
        <div className="card-label">Параметры для inject</div>
        {files.length > 1 && !allDone && (
